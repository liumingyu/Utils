...
+(BOOL)isPhone:(NSString *)phone{
    ...
    BOOL isRe = [predicate evaluateWithObject:phone];
    return isRe;
...
+(BOOL)isIdentify:(NSString*)identifier{
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",regex];
    BOOL isRe = [predicate evaluateWithObject:identifier];
    if (!isRe) {
        //身份证号码格式不对
        return NO;
    }
    ...
    for (int i = 0; i < weightingArray.count; i++) {//将前17位数字和加权因子相乘的结果相加
        NSString *subStr = [identifier substringWithRange:NSMakeRange(i, 1)];
    ...
    NSString *idCardLast = [identifier.uppercaseString substringFromIndex:17]; //获取身份证最后一位
